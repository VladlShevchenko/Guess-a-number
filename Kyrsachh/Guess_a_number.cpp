...
void play_with_friend() {
	...
	cin >> num;																		// считываем введённую переменную num 
	...
	for (int prop = 0; prop < 100; prop++) 
		cout << endl;						
	cout << setw(70) << "Second player,try to guess a number in " << amount_of_tries << " tries!" << endl; // задаём пропуск на 70 пропусков и выводим кол попыток
	...
	do
	{
		...
		if (guess < 1 || guess > 100)												// если предположение пользователя выходит за заданые границы выводим предупреждение
			...
			if (guess - num < 5) cout << "so close, but ";							// если предположение пользователя меньше за истинное и оч близко к нему выводим 2ю подсказку
			cout << "too much, let`s try one more time\n\n";						// выводим подсказку
			PlaySoundA((LPCSTR)"no-6.wav", NULL, SND_FILENAME | SND_ASYNC);			// проигрываем звук no-6.wav
			tries++;																// добавляем +одну попытку
		}
